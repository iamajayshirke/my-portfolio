        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            ...
            link.href.startsWith('#') ? (
              <a key={link.name} href={link.href} className="text-sm font-medium text-slate-400 hover:text-blue-400 transition-colors">
                {link.name}
              </a>
            ) : (
              <Link key={link.name} to={link.href} className="text-sm font-medium text-slate-400 hover:text-blue-400 transition-colors">
                {link.name}
              </Link>
            )
          ))}
          ...
          {isLanding && (
            <Link to="/admin-portal-xyz" className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-full transition-all">
              Get Started
            </Link>
          )}

          {isAdmin && (
            <button className="flex items-center gap-2 text-red-400 hover:text-red-300 text-sm font-medium">
              <LogOut size={16} /> Logout
            </button>
          )}

          {isPortfolio && (
            <Link to="/admin-portal-xyz" className="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-300">
               <UserIcon size={18} />
            </Link>
          )}
        </div>
